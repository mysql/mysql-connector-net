...
{
  public class SetUpEntityTests : SetUpClass
  {
    ...
    public SetUpEntityTests()      
    {
      ...
      MySqlScript script = new MySqlScript(base.conn);
      script.Query = schema;
      script.Execute();

      // now create our procs
      schema = r.GetString("procs");
      script = new MySqlScript(base.conn);
      script.Delimiter = "$$";
      script.Query = schema;
      script.Execute();
      ...
      MySqlCommand cmd = new MySqlCommand("DROP DATABASE IF EXISTS `modeldb`", base.rootConn);
      cmd.ExecuteNonQuery();
    }
