...
namespace MySql.Data.Entity
{
    ...
    public class MySqlMigrationSqlGenerator : MigrationSqlGenerator
    {
        ...
        protected virtual MigrationStatement Generate(RenameColumnOperation op)
        {
            ...
            var sql = $@"
set @tableName = '{TrimSchemaPrefix(op.Table)}';
set @columnName = '{op.Name}';
set @newColumnName = '{op.NewName}';
set @dummyQuery = 'select 0';
...
set @columnType := (
	select
		concat(
            ...
            if(lower(`IS_NULLABLE`) = 'no', ' not null', ' null'),
			if(`COLUMN_DEFAULT` is null, '', concat(' default ''', `COLUMN_DEFAULT`, '''')),
			if(`COLUMN_COMMENT` is null, '', concat(' comment ''', `COLUMN_COMMENT`, ''''))
		)
	from
		`information_schema`.`columns`
	where
		`TABLE_SCHEMA` = schema() and `TABLE_NAME` = @tableName and `COLUMN_NAME` = @columnName
);

set @dropConstraintsStatement := (
	select
		group_concat(
			concat(
				'alter table `',
	...
	where
		`k`.`TABLE_SCHEMA` = schema() and `k`.`TABLE_NAME` = @tableName and `k`.`COLUMN_NAME` = @columnName and `k`.`REFERENCED_COLUMN_NAME` is not null
	order by
		`k`.`ORDINAL_POSITION` desc
);
set @dropConstraintsStatement = ifnull(@dropConstraintsStatement, @dummyQuery);

set @addConstraintsStatement := (
	select
        group_concat(
			concat(
				'alter table `',
				...
				'`)',
                if(`c`.`DELETE_RULE` is null, '', concat(' ON DELETE ', `c`.`DELETE_RULE`)),
                if(`c`.`UPDATE_RULE` is null, '', concat(' ON UPDATE ', `c`.`UPDATE_RULE`)),
                '; '
			)
		)
    from
    ...
    left join
		`information_schema`.`REFERENTIAL_CONSTRAINTS` `c` on `c`.`CONSTRAINT_CATALOG` = `k`.`CONSTRAINT_CATALOG` and `c`.`CONSTRAINT_SCHEMA` = `k`.`CONSTRAINT_SCHEMA` and `c`.`CONSTRAINT_NAME` = `k`.`CONSTRAINT_NAME`
	where
		`k`.`TABLE_SCHEMA` = schema() and `k`.`TABLE_NAME` = @tableName and `k`.`COLUMN_NAME` = @columnName and `k`.`REFERENCED_COLUMN_NAME` is not null
	order by
		`k`.`ORDINAL_POSITION` asc
);
set @addConstraintsStatement = ifnull(@addConstraintsStatement, @dummyQuery);

set @renameColumnStatement := (select concat('alter table `', @tableName, '` change `', @columnName, '` `', @newColumnName, '` ', @columnType, '; '));

prepare statement from @dropConstraintsStatement;
execute statement;
deallocate prepare statement;

prepare statement from @renameColumnStatement;
execute statement;
deallocate prepare statement;

prepare statement from @addConstraintsStatement;
execute statement;
deallocate prepare statement;
";
            return new MigrationStatement { Sql = sql };
        }
        ...
        protected virtual MigrationStatement Generate(RenameIndexOperation op)
        {
            ...
            {
                Sql = $@"ALTER TABLE {TrimSchemaPrefix(op.Table)} RENAME INDEX `{op.Name}` TO `{op.NewName}`;"
            };
        }
        ...
        {
            StringBuilder sb = new StringBuilder();
            sb = sb.AppendFormat("alter table `{0}` drop foreign key `{1}`", TrimSchemaPrefix(op.DependentTable), TrimSchemaPrefix(op.Name));
            return new MigrationStatement { Sql = sb.ToString() };
        }
        ...
        protected virtual MigrationStatement Generate(DropPrimaryKeyOperation op)
        {
            ...
            return new MigrationStatement { Sql = sb + " alter table `" + TrimSchemaPrefix(op.Table) + "` drop primary key " };
        }
