...
namespace MySql.EntityFrameworkCore.Scaffolding.Internal
{
  ...
  AND {0};";
   ...
   GROUP BY `TABLE_SCHEMA`, `TABLE_NAME`, `INDEX_NAME`, `NON_UNIQUE`;";
    ...
    WHERE `REFERENTIAL_CONSTRAINTS`.`CONSTRAINT_CATALOG` = `KEY_COLUMN_USAGE`.`CONSTRAINT_CATALOG`
	...
	AND `REFERENTIAL_CONSTRAINTS`.`CONSTRAINT_NAME` = `KEY_COLUMN_USAGE`.`CONSTRAINT_NAME`) AS `DELETE_RULE`
