...
namespace MySql.Data.Types
{
  internal struct MySqlInt16 : IMySqlValue
  {
    ...
    /// <summary>
    /// Writes the 16-bit integer value to the MySQL packet, either in binary or text format.
    /// </summary>
    ...
    /// <summary>
    /// Asynchronously writes the 16-bit integer value to the MySQL packet, either in binary or text format.
    /// </summary>
    /// <param name="packet">The MySQL packet to write the value to.</param>
    /// <param name="binary">Indicates whether to write the value in binary (<c>true</c>, 2 bytes) or text (<c>false</c>, string) format.</param>
    /// <param name="val">The integer value to write.</param>
    ...
    async Task IMySqlValue.WriteValueAsync(MySqlPacket packet, bool binary, object val, int length)
    {
      ...
      if (binary)
        await packet.WriteIntegerAsync((long)v, 2).ConfigureAwait(false);
      else
        await packet.WriteStringNoNullAsync(v.ToString(CultureInfo.InvariantCulture)).ConfigureAwait(false);
    }

    /// <summary>
    /// Reads the 16-bit integer value from the MySQL packet.
    /// </summary>
    ...
    /// <returns>A new <see cref="MySqlInt16"/> instance representing the read value, or a null instance if <paramref name="nullVal"/> is <c>true</c>.</returns>
    ...
    /// <summary>
    /// Asynchronously reads the 16-bit integer value from the MySQL packet.
    /// </summary>
    /// <param name="packet">The MySQL packet to read from.</param>
    /// <param name="length">The length of the value to read. A value of -1 indicates binary format (2 bytes).</param>
    /// <param name="nullVal">Indicates if the value is null.</param>
    ...
    async Task<IMySqlValue> IMySqlValue.ReadValueAsync(MySqlPacket packet, long length, bool nullVal)
    {
      ...
      if (length == -1)
        ...
        return new MySqlInt16(Int16.Parse(await packet.ReadStringAsync(length).ConfigureAwait(false), CultureInfo.InvariantCulture));
    }
