...
namespace MySqlX.Data.Tests
{
  public class ClientSideFailoverTests : BaseTest
  {
    ...
    public void SequentialMethodWithBasicFormatConnectionString()
    {
      ...
      using (var session = MySQLX.GetSession("server=10.10.10.10, localhost;port=" + XPort + ";uid=test;password=test;connecttimeout=" + connectionTimeout))
      {
        Assert.Equal(SessionState.Open, session.InternalSession.SessionState);
      }
      ...
      Exception ex = Assert.Throws<TimeoutException>(() => MySQLX.GetSession("server= 10.10.10.10, 20.20.20.20 ;port=" + XPort + ";uid=test;password=test;connecttimeout=" + connectionTimeout));
      Assert.Equal("All server connection attempts were aborted. Timeout of 1000 milliseconds was exceeded for each selected server.", ex.Message);

      // Providing port number as part of the host name.
      ex = Assert.Throws<ArgumentException>(() => MySQLX.GetSession("server= 10.10.10.10:33060, 20.20.20.20:33060 ;port=" + XPort + ";uid=test;password=test;connecttimeout=" + connectionTimeout));
      ...
      // Use default port.
      //using (var session = MySQLX.GetSession("server=localhost;uid=test;password=test;connecttimeout=" + connectionTimeout + ";"))
      //{
      //  Assert.Equal(SessionState.Open, session.InternalSession.SessionState);
    ...
    public void SequentialMethodWithUriFormatConnectionString()
    {
      ...
      //using (var session = MySQLX.GetSession("mysqlx://test:test@[127.0.0.1]?connecttimeout=" + connectionTimeout))
      //{
      //  Assert.Equal(SessionState.Open, session.InternalSession.SessionState);
      //}
    ...
    public void SequentialMethodWithAnonymousTypes()
    {
      ...
      // Use default port.
      //using (var session = MySQLX.GetSession(new { server = "localhost", uid = uid, password = password, connecttimeout = connectionTimeout }))
      //{
      //  Assert.Equal(SessionState.Open, session.InternalSession.SessionState);
      //}
      ...
      // Multiple hosts. All attempts fail.
      Exception ex = Assert.Throws<TimeoutException>(() => MySQLX.GetSession(new { server = "10.10.10.10, 20.20.20.20", port = XPort, uid = uid, password = password, connecttimeout = connectionTimeout }));
      Assert.Equal("All server connection attempts were aborted. Timeout of 1000 milliseconds was exceeded for each selected server.", ex.Message);
    ...
    public void PriorityMethodWithBasicFormatConnectionString()
    {
      ...
      // Use default port.
      //using (var session = MySQLX.GetSession("server=(address=localhost,priority=100);uid=test;password=test;connecttimeout=" + connectionTimeout))
      //{
      //  Assert.Equal(SessionState.Open, session.InternalSession.SessionState);
      //  Assert.Equal("localhost", session.Settings.Server);
      //}
    ...
    public void PriorityMethodWithAnonymousTypes()
    {
      ...
      // Use default port.
      //using (var session = MySQLX.GetSession(new { server = "(address=127.0.0.1,priority=100)", uid = uid, password = password, connecttimeout = connectionTimeout }))
      //{
      //  Assert.Equal(SessionState.Open, session.InternalSession.SessionState);
      //  Assert.Equal("127.0.0.1", session.Settings.Server);
      //}
